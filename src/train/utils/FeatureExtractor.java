...
public class FeatureExtractor {
	 ...
	 */
	public Vector<Double> getFeature(FetchedDocument fetchedDocument) throws UnsupportedEncodingException {
	...
	private String getContent(FetchedDocument fetchedDocument)
			throws UnsupportedEncodingException {
		String content = null;
		content = new String(fetchedDocument.getDocumentContent(),
				fetchedDocument.getContentCharset());
		return content;
	}

	private int getMaxLineBlockLength(String rawContent) {
		return getMaxLineBlockText(rawContent).length();
	}

	public String getMaxLineBlockText(FetchedDocument fetchedDocument) throws UnsupportedEncodingException {
	...
	private String getMaxLineBlockText(String content) {
		...
		for (int i = 1; i < textList.size(); i++) {
			if (textBeginList.get(i) == textEndList.get(i - 1) + 1
				...
				if (textEndList.get(i - 1) == textBeginList.get(i - 1)
						+ blockLines
